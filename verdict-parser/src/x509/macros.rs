...
/// NOTE: we have the restriction that an Choice combinator cannot
            ...
            ::vstd::prelude::verus! {
                ...
                mod [< internal_ $name >] {
                    ...
                    mapper! {
                        ...
                        spec SpecValue with <$(<<$field_combinator_type as View>::V as SpecCombinator>::Type),*>;
                        exec Value<'a> with <$(<$field_combinator_type as Combinator<'a, &'a [u8], Vec<u8>>>::SType),*>;
                        owned ValueOwned with <$(<$field_combinator_type as Combinator<'static, &'static [u8], Vec<u8>>>::Type),*>;
...
macro_rules! gen_inner_combinator_type {
    ...
    ((, $first:ty) $(; ($($modifier:ident $(($modifier_arg:expr))?)?, $rest:ty))*) => {
        ($first, gen_inner_combinator_type!($(($($modifier $(($modifier_arg))?)?, $rest));*))
    };
    ...
    ((default($_:expr), $first:ty) $(; ($($modifier:ident $(($modifier_arg:expr))?)?, $rest:ty))*) => {
        Default<<$first as Combinator<'static, &'static [u8], Vec<u8>>>::Type, $first, gen_inner_combinator_type!($(($($modifier $(($modifier_arg))?)?, $rest));*)>
    };
...
macro_rules! gen_inner_combinator {
    ...
    ((, $first:expr) $(; ($($modifier:ident $(($modifier_arg:expr))?)?, $rest:expr))*) => {
        Pair::new($first, gen_inner_combinator!($(($($modifier $(($modifier_arg))?)?, $rest));*))
    };
...
macro_rules! gen_inner_combinator_poly_result_type {
    ...
    ((, $first:ident) $(; ($($modifier:ident $(($modifier_arg:expr))?)?, $rest:ident))*) => {
        ($first, gen_inner_combinator_poly_result_type!($(($($modifier $(($modifier_arg))?)?, $rest));*))
    };

    ((optional, $first:ident) $(; ($($modifier:ident $(($modifier_arg:expr))?)?, $rest:ident))*) => {
        (OptionDeep<$first>, gen_inner_combinator_poly_result_type!($(($($modifier $(($modifier_arg))?)?, $rest));*))
    };
...
macro_rules! gen_backward_body {
    ...
    ($src:expr ; ($($modifier:ident $(($modifier_arg:expr))?)?, $first:ident) $(, ($($rest_modifier:ident $(($rest_modifier_arg:expr))?)?, $rest:ident))*) => {
        ($src.$first, gen_backward_body!($src ; $(($($rest_modifier $(($rest_modifier_arg))?)?, $rest)),*))
    };
}
        ...
        ::paste::paste! {
            ::vstd::prelude::verus! {
                ...
                mod [< internal_ $name >] {
                    ...
                    impl<$lt> Continuation<&$lt $input_type> for Cont {
                        ...
                        gen_match_continuation_apply! {
                            Mapper, &$lt $input_type;
                            $(($value, $spec_value, $combinator),)*
                            (, $last_combinator)
                        }

                        open spec fn requires(&self, i: &$lt $input_type) -> bool {
                            true
                        }

                        open spec fn ensures(&self, i: &$lt $input_type, o: Self::Output) -> bool {
                            &&& o@ == Self::spec_apply(i@)
                        }
                    }
                    ...
                    mapper! {
                        ...
                        spec SpecValue with <
                            $(<<$combinator_type as View>::V as SpecCombinator>::Type,)*
                            <<$last_combinator_type as View>::V as SpecCombinator>::Type
                        >;
                        exec Value<'a> with <
                            $(<$combinator_type as Combinator<'a, &'a [u8], Vec<u8>>>::SType,)*
                            <$last_combinator_type as Combinator<'a, &'a [u8], Vec<u8>>>::SType
                        >;
                        owned ValueOwned with <
                            $(<$combinator_type as Combinator<'static, &'static [u8], Vec<u8>>>::Type,)*
                            <$last_combinator_type as Combinator<'static, &'static [u8], Vec<u8>>>::Type
                        >;
...
macro_rules! gen_match_continuation_apply {
    ($mapper:expr, $input_type:ty; $(($value:expr, $spec_value:expr, $combinator:expr),)* (, $last_combinator:expr)) => {
        ::vstd::prelude::verus! {
            #[inline(always)]
            fn apply(&self, i: $input_type) -> (o: Self::Output) {
...
/// ```
///
/// This essentially generates an `Choice` combinator over all the choices
///
        ...
        ::paste::paste! {
            ::vstd::prelude::verus! {
                ...
                mod [< internal_ $name >] {
                    ...
                    pub type SpecValue = Seq<<<$combinator_type as View>::V as SpecCombinator>::Type>;
                    pub type Value<'a> = VecDeep<<$combinator_type as Combinator<'a, &'a [u8], Vec<u8>>>::SType>;
                    pub type ValueOwned = VecDeep<<$combinator_type as Combinator<'static, &'static [u8], Vec<u8>>>::Type>;
                }
            }
        }
    };
