...
mapper! {
    ...
    for <Id, Param>
    from ExtensionFrom where type ExtensionFrom<Id, Param> = (Id, (OptionDeep<bool>, Param));
...
impl Continuation<ObjectIdentifierValue> for ExtensionCont {
    type Output = Optional<Wrapped<ASN1<Boolean>>, <ExtensionParamCont as Continuation>::Output>;

    fn apply(&self, i: ObjectIdentifierValue) -> (o: Self::Output) {
        Optional(new_wrapped(ASN1(Boolean)), ExtensionParamCont.apply(i))
    }

    open spec fn requires(&self, i: ObjectIdentifierValue) -> bool {
        true
    }

    open spec fn ensures(&self, i: ObjectIdentifierValue, o: Self::Output) -> bool {
        o@ == Self::spec_apply(i@)
    }
}
